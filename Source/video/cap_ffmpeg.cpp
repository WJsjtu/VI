...
public:
	...
	CvCapture_FFMPEG_proxy(const std::string& filename, const VideoCaptureParameters& params)
		: ffmpegCapture(NULL)
	{
	...
	virtual double getProperty(int propId) const override
	{
		return ffmpegCapture ? cvGetCaptureProperty_FFMPEG(ffmpegCapture, propId) : 0;
	}
	virtual bool setProperty(int propId, double value) override
	{
	...
	virtual bool retrieveFrame(int, IVideoCaptureFrame& frame) override
	{
		...
		if (!cvRetrieveFrame_FFMPEG(ffmpegCapture, &data, &step, &width, &height, &cn))
			return false;
