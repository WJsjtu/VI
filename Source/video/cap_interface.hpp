...
public:
    ...
    void add(int key, int value) { params_.emplace_back(key, value); }

    bool has(int key) const {
        auto it = std::find_if(params_.begin(), params_.end(), [key](const VideoParameter& param) { return param.key == key; });
    ...
    bool warnUnusedParameters() const {
        ...
        for (const auto& param : params_) {
            if (!param.isConsumed) {
                found = true;
                std::string str = fmt::sprintf("%lld / 0x%016llx", (long long)param.value, (long long)param.value);
                CV_LOG_WARN(NULL, "VIDEOIO: unused parameter: [" << param.key << "]=" << str);
            }
        }
        return found;
    }
...
public:
    ...
    virtual bool retrieveFrame(int, IVideoCaptureFrame&, bool) = 0;
