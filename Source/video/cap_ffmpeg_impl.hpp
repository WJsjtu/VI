...
bool CvCapture_FFMPEG::open(const char* _filename, const VideoCaptureParameters& params)
{
	...
	if (err < 0)
	{
		CV_WARN(NULL, "Error opening file");
		CV_WARN(NULL, _filename);
		goto exit_func;
	}
	err = avformat_find_stream_info(ic, NULL);
	if (err < 0)
	{
		CV_WARN(NULL, "Could not find codec parameters");
		goto exit_func;
	}
...
bool CvCapture_FFMPEG::setRaw()
{
	if (!rawMode)
	{
		if (frame_number != 0)
		{
			CV_WARN(NULL, "Incorrect usage: do not grab frames before .set(CAP_PROP_FORMAT, -1)");
		}
...
bool CvCapture_FFMPEG::processRawPacket()
{
    ...
    && LIBAVCODEC_BUILD >= CALC_FFMPEG_VERSION(55, 34, 1)  // libav v10+
		...
		if (filterName)
		{
			...
			if (!bsf)
			{
				CV_WARN(NULL, "Bitstream filter is not available: " << filterName);
				return false;
			}
			int err = av_bsf_alloc(bsf, &bsfc);
			if (err < 0)
			{
				CV_WARN(NULL, "Error allocating context for bitstream buffer");
				return false;
			}
			...
			if (err < 0)
			{
				CV_WARN(NULL, "Error initializing bitstream buffer");
				return false;
			}
#else
	...
	if (bsfc)
	{
		...
		if (err < 0)
		{
			CV_WARN(NULL, "Packet submission for filtering failed");
			return false;
		}
		err = av_bsf_receive_packet(bsfc, &packet_filtered);
		if (err < 0)
		{
			CV_WARN(NULL, "Filtered packet retrieve failed");
			return false;
		}
#else
