    ...
    STDMETHODIMP SampleCB(double /*Time*/, IMediaSample* pSample) {
        ...
        if (hr == S_OK) {
            ...
            if (latestBufferLength == numBytes) {
                ...
                Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR: SampleCB() - buffer sizes do not match\n");
            }
        }
...
void videoDevice::setSize(int w, int h) {
    if (sizeSet) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Warning, "SETUP: Error device size should not be set more than once \n");
    } else {
        width = w;
        height = h;
        videoSize = w * h * 3;
...
void videoDevice::destroyGraph() {
    ...
    while (hr == NOERROR) {
        ...
        if (FAILED(hr)) {
            if (verbose) Utils::LoggerPrintf(VI::LogLevel::Warning, "SETUP: pGraph->EnumFilters() failed. \n");
            return;
        }
        ...
        if (pEnum->Next(1, &pFilter, NULL) == S_OK) {
            ...
            if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: removing filter %ls...\n", Utils::WideCharToMultiByteString(FilterInfo.achName).c_str());
            hr = pGraph->RemoveFilter(pFilter);
            if (FAILED(hr)) {
                if (verbose) Utils::LoggerPrintf(VI::LogLevel::Warning, "SETUP: pGraph->RemoveFilter() failed. \n");
                return;
            }
            if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: filter removed %ls  \n", Utils::WideCharToMultiByteString(FilterInfo.achName).c_str());
...
videoDevice::~videoDevice() {
    if (setupStarted) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "\nSETUP: Disconnecting device %i\n", myID);
    } else {
        if (sgCallback) {
            sgCallback->Release();
            delete sgCallback;
    ...
    if ((sgCallback) && (pGrabber)) {
        ...
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: freeing Grabber Callback\n");
        sgCallback->Release();
    ...
    if ((pControl)) {
        HR = pControl->Pause();
        if (FAILED(HR))
            if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "ERROR - Could not pause pControl\n");

        HR = pControl->Stop();
        if (FAILED(HR))
            if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "ERROR - Could not stop pControl\n");
    }
    ...
    if ((pDestFilter)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: freeing Renderer \n");
        (pDestFilter)->Release();
        (pDestFilter) = 0;
    }
    if ((pVideoInputFilter)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: freeing Capture Source \n");
        (pVideoInputFilter)->Release();
        (pVideoInputFilter) = 0;
    }
    if ((pGrabberF)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: freeing Grabber Filter  \n");
        (pGrabberF)->Release();
        (pGrabberF) = 0;
    }
    if ((pGrabber)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: freeing Grabber  \n");
        (pGrabber)->Release();
        (pGrabber) = 0;
    }
    if ((pControl)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: freeing Control   \n");
        (pControl)->Release();
        (pControl) = 0;
    }
    if ((pMediaEvent)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: freeing Media Event  \n");
        (pMediaEvent)->Release();
        (pMediaEvent) = 0;
    }
    if ((streamConf)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: freeing Stream  \n");
        (streamConf)->Release();
        (streamConf) = 0;
    }

    if ((pAmMediaType)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: freeing Media Type  \n");
    ...
    if ((pCaptureGraph)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: freeing Capture Graph \n");
        (pCaptureGraph)->Release();
        (pCaptureGraph) = 0;
    }
    if ((pGraph)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: freeing Main Graph \n");
        (pGraph)->Release();
        (pGraph) = 0;
    }

    if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: Device %i disconnected and freed\n\n", myID);
}
...
void videoInput::setUseCallback(bool useCallback) {
    if (callbackSetCount == 0) {
        ...
        Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR: setUseCallback can only be called before setup\n");
    }
}
...
bool videoInput::setFormat(int deviceNumber, int format) {
    ...
    if (format >= 0 && format < VI_NUM_FORMATS) {
        ...
        if (VDList[deviceNumber]->specificFormat) {
            ...
            if (FAILED(hr)) {
                Utils::LoggerPrintf(VI::LogLevel::Warning, "SETUP: couldn't set requested format\n");
            } else {
                long lValue = 0;
...
const std::wstring& videoInput::getUniqueDeviceName(int deviceID) {
    ...
    if (deviceID < 0 || deviceID >= (int)deviceUniqueNames.size()) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR: Unknown device unique name - device index is out of range\n");
        return dummy;
    }
...
int videoInput::getDeviceIDFromUniqueName(const std::wstring& uniqueName) {
    ...
    if (iter != deviceUniqueNames.end())
        ...
        Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR: Unknown unique device name requested\n");
...
int videoInput::listDevices(bool silent) {
    ...
    if (!silent) Utils::LoggerPrintf(VI::LogLevel::Info, "\nVIDEOINPUT SPY MODE!\n\n");
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (hr == S_OK) {
            if (!silent) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: Looking For Capture Devices\n");
            ...
            while (pEnum->Next(1, &pMoniker, NULL) == S_OK) {
                if (deviceCounter >= VI_MAX_CAMERAS) {
                    if (!silent) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: Too many video inputs! Stop listing at %d\n", deviceCounter);
                    pMoniker->Release();
                ...
                if (SUCCEEDED(hr)) {
                    ...
                    if (!silent) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: %i) %s \n", deviceCounter, deviceNames[deviceCounter]);
        ...
        if (!silent) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: %i Device(s) found\n\n", deviceCounter);
    }
        ...
        if (bCallback) {
            ...
            if (hr == S_OK) {
                ...
                if (numBytes == bufferSize) {
                    ...
                    if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR: GetPixels() - bufferSizes do not match!\n");
                }
            } else {
                if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR: GetPixels() - Unable to grab frame for device %i\n", id);
            }
        }
    }
...
bool videoInput::isFrameNew(int id) {
    ...
    if (freeze && VDList[id]->autoReconnect) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR: Device seems frozen - attempting to reconnect\n");
        if (!restartDevice(VDList[id]->myID)) {
            if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR: Unable to reconnect to device\n");
        } else {
            if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SUCCESS: Able to reconnect to device\n");
        }
    }
...
bool videoInput::getVideoSettingFilter(int deviceID, long Property, long& min, long& max, long& SteppingDelta, long& currentValue, long& flags, long& defaultValue) {
    ...
    if (FAILED(hr)) {
        Utils::LoggerPrintf(VI::LogLevel::Error, "setVideoSetting - getDevice Error\n");
        return false;
    }
    ...
    if (FAILED(hr)) {
        Utils::LoggerPrintf(VI::LogLevel::Error, "setVideoSetting - QueryInterface Error\n");
        if (VD->pVideoInputFilter) VD->pVideoInputFilter->Release();
        if (VD->pVideoInputFilter) VD->pVideoInputFilter = NULL;
        return false;
    }

    if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "Setting video setting %ld.\n", Property);

    pAMVideoProcAmp->GetRange(Property, &min, &max, &SteppingDelta, &defaultValue, &flags);
    if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "Range for video setting %ld: Min:%ld Max:%ld SteppingDelta:%ld Default:%ld Flags:%ld\n", Property, min, max, SteppingDelta, defaultValue, flags);
...
bool videoInput::setVideoSettingFilterPct(int deviceID, long Property, float pctValue, long Flags) {
    ...
    if (range == stepAmnt) {
        ...
        Utils::LoggerPrintf(VI::LogLevel::Info, "RASTER - pctValue is %f - value is %i - step is %i - mod is %i - rasterValue is %i\n", pctValue, value, stepAmnt, mod, rasterValue);
    }
...
bool videoInput::setVideoSettingFilter(int deviceID, long Property, long lValue, long Flags, bool useDefaultValue) {
    ...
    if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "Current value: %ld Flags %ld (%s)\n", CurrVal, CapsFlags, (CapsFlags == 1 ? "Auto" : (CapsFlags == 2 ? "Manual" : "Unknown")));
...
bool videoInput::setVideoSettingCamera(int deviceID, long Property, long lValue, long Flags, bool useDefaultValue) {
    ...
    if (isDeviceSetup(deviceID)) {
        ...
        if (FAILED(hr)) {
            Utils::LoggerPrintf(VI::LogLevel::Error, "Error\n");
            return false;
        } else {
...
bool videoInput::comInit() {
    ...
    if (comInitCount == 0) {
        ...
        if (hr == RPC_E_CHANGED_MODE) {
            if (verbose) Utils::LoggerPrintf(VI::LogLevel::Warning, "SETUP - COM already setup - threaded VI might not be possible\n");
        }
    }
...
bool videoInput::setup(int deviceNumber) {
    ...
    if (deviceNumber > devicesFound - 1) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Warning, "SETUP: device[%i] not found - you have %i devices available\n", deviceNumber, devicesFound);
        if (devicesFound >= 0)
            if (verbose) Utils::LoggerPrintf(VI::LogLevel::Warning, "SETUP: this means that the last device you can use is device[%i] \n", devicesFound - 1);
        return false;
    }

    if (VDList[deviceNumber]->readyToCapture) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Warning, "SETUP: can't setup, device %i is currently being used\n", VDList[deviceNumber]->myID);
        return false;
    }
    ...
    if (iSize == sizeof(VIDEO_STREAM_CONFIG_CAPS)) {
        ...
        for (int iFormat = 0; iFormat < iCount; iFormat++) {
            ...
            if (SUCCEEDED(hr)) {
                ...
                // if(verbose)Utils::LoggerPrintf(VI::LogLevel::Info, "min is %i %i max is %i %i - res is %i %i \n", scc.MinOutputSize.cx, scc.MinOutputSize.cy,  scc.MaxOutputSize.cx,  scc.MaxOutputSize.cy, stepX, stepY);
                // if(verbose)Utils::LoggerPrintf(VI::LogLevel::Info, "min frame duration is %i  max duration is %i\n", scc.MinFrameInterval, scc.MaxFrameInterval);
...
int videoInput::start(int deviceID, videoDevice* VD) {
    ...
    if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: Setting up device %i\n", deviceID);
    ...
    {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR - Could not create the Filter Graph Manager\n");
        return hr;
    }
    ...
    if (FAILED(hr)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR - Could not add the graph builder!\n");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if (FAILED(hr)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR - Could not set filtergraph\n");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if (FAILED(hr)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR - Could not create the Media Control object\n");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if (SUCCEEDED(hr)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: %s\n", VD->nDeviceName);
        hr = VD->pGraph->AddFilter(VD->pVideoInputFilter, VD->wDeviceName);
    } else {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR - Could not find specified video device\n");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if (FAILED(hr)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "SETUP: Couldn't find preview pin using SmartTee\n");
    } else {
    ...
    if (VD->useCrossbar) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: Checking crossbar\n");
    ...
    if (FAILED(hr)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR: Couldn't config the stream!\n");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if (FAILED(hr)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR: Couldn't getFormat for pAmMediaType!\n");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if (customSize) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: Default Format is set to %i by %i \n", currentWidth, currentHeight);

        char guidStr[8];
        getMediaSubtypeAsString(requestedMediaSubType, guidStr);

        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: trying requested format %s @ %i by %i\n", guidStr, VD->tryWidth, VD->tryHeight);
        ...
        if (!foundSize) {
            for (int i = 0; i < VI_NUM_TYPES; i++) {
                ...
                if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: trying format %s @ %i by %i\n", guidStr, VD->tryWidth, VD->tryHeight);
        ...
        if (foundSize == false) {
            if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "SETUP: couldn't find requested size - searching for closest matching size\n");
            ...
            if (closestWidth != -1 && closestHeight != -1) {
                ...
                if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: closest supported size is %s @ %i %i\n", guidStr, closestWidth, closestHeight);
    ...
    if (FAILED(hr)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "Could not Create Sample Grabber - CoCreateInstance()\n");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if (FAILED(hr)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "Could not add Sample Grabber - AddFilter()\n");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if (FAILED(hr)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR: Could not query SampleGrabber\n");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if (bCallback) {
        ...
        if (FAILED(hr)) {
            if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR: problem setting callback\n");
            stopDevice(deviceID);
            return hr;
        } else {
            if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: Capture callback set\n");
        }
    }
    ...
    if (VD->streamConf) {
        ...
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR: connecting device - prehaps it is already being used?\n");
        stopDevice(deviceID);
        return S_FALSE;
    }
    ...
    if (FAILED(hr)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR: Could not create filter - NullRenderer\n");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if (FAILED(hr)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR: Could not add filter - NullRenderer\n");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if (FAILED(hr)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR: Could not connect pins - RenderStream()\n");
        stopDevice(deviceID);
        return hr;
    }
    ...
    // EXP - lets try setting the sync source to null - and make it run as fast as possible
    {
        ...
        if (FAILED(hr)) {
            if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR: Could not get IID_IMediaFilter interface\n");
        } else {
    ...
    if (FAILED(hr)) {
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Error, "ERROR: Could not start graph\n");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: Device is setup and ready to capture.\n\n");
...
HRESULT videoInput::routeCrossbar(ICaptureGraphBuilder2** ppBuild, IBaseFilter** pVidInFilter, int conType, GUID captureMode) {
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: You are not a webcam! Setting Crossbar\n");
        pXBar1->Release();
        ...
        while (pIndex < lInpin) {
            ...
            if (pType == conType) {
                if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: Found Physical Interface");

                switch (conType) {
                    case PhysConn_Video_Composite:
                        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, " - Composite\n");
                        break;
                    case PhysConn_Video_SVideo:
                        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, " - S-Video\n");
                        break;
                    case PhysConn_Video_Tuner:
                        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, " - Tuner\n");
                        break;
                    case PhysConn_Video_USB:
                        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, " - USB\n");
                        break;
                    case PhysConn_Video_1394:
                        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, " - Firewire\n");
                        break;
                }
        ...
        if (foundDevice) {
            ...
            if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: Didn't find specified Physical Connection type. Using Defualt. \n");
        }
        ...
        if (verbose) Utils::LoggerPrintf(VI::LogLevel::Info, "SETUP: You are a webcam or snazzy firewire cam! No Crossbar needed\n");
        return hr;
    }
